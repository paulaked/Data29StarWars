...
#getting properties for all the starships and changing the pilot urls to pilot names and then changing names to objectID


def get_properties_name_id():
    for url in starship_url():
        ...
        if properties['new pilots'] != []:
            ...
            print(properties)
print(get_properties_name_id())

#drop existing starship collection
db.drop_collection(("starships"))

#create starships collection on Mongodb assuming it doesnt already exist
db.create_collection("starships")

#adding the starships to the starship collection on Mongodb
def add_to_mongodb():
    for i in get_properties_name_id():
        db.insert_one(i)
